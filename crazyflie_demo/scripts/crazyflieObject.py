...
class CrazyflieObject(object):
	def __init__(self, name, world_size_x, world_size_y): # world_size_x = WORLD_RANGE["X"][0] # world_size_y = WORLD_RANGE["Y"][0]
		self._name = name
		self._status = "Landed"
	...
	def getPosition(self):
		...
		return [real_pos[0] - self.world_size_x, real_pos[1] - self.world_size_y, real_pos[2]]
	def getStatus(self):
		return self._status
	def getSpeed(self):
	...
	def goTo(self, x, y, duration=1):
		self._cf.goTo(goal = [x + self.world_size_x, y + self.world_size_y, FLIGHT_HEIGHT], yaw=0.0, duration=duration, relative=False)
